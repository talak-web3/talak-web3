...
export async function doctorCommand(options: DoctorOptions = {}) {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    const hasTalakWeb3 =
      packageJson.dependencies?.["talak-web3"] || packageJson.devDependencies?.["talak-web3"];
    if (hasTalakWeb3) {
      results.push({
        name: "talak-web3 dependency",
        status: "pass",
        message: `Version: ${hasTalakWeb3}`,
      });
    } else {
      results.push({
        name: "talak-web3 dependency",
        status: "fail",
        message: "talak-web3 not installed",
        fix: 'Run "npm install talak-web3"',
      });
    }
  }
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (hasPrivKey && hasPubKey) {
      results.push({
        ...
        status: "pass",
        message: "JWT_PRIVATE_KEY and JWT_PUBLIC_KEY configured",
      });
    } else {
      results.push({
        name: "JWT Asymmetric Keys",
        status: "fail",
        message: "JWT_PRIVATE_KEY or JWT_PUBLIC_KEY missing or using default",
        fix: "Generate and set RS256 keys in .env",
      });
    }
    ...
    if (hasRedisUrl) {
      ...
      results.push({
        ...
        fix: "Set REDIS_URL in .env (Mandatory for production)",
      });
    }
  } else {
    results.push({
  ...
  if (fs.existsSync(tsConfigPath)) {
    ...
    results.push({
      ...
      fix: "Create a tsconfig.json for TypeScript",
    });
  }
