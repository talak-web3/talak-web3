  ...
  async auditSecurity(): Promise<{
    ...
    try {
      ...
      if (configPolicy["maxmemory-policy"] !== "noeviction") {
        issues.push(
          `WARNING: Redis eviction policy is ${configPolicy["maxmemory-policy"]} instead of noeviction`,
        );
      ...
      if (aofConfig["appendonly"] !== "yes") {
        issues.push(
          "WARNING: Redis AOF persistence is disabled - nonce data may be lost on restart",
        );
      ...
      if (fsyncConfig["appendfsync"] === "no") {
        ...
        recommendations.push(
          'Set appendfsync to "everysec" for balance of performance and durability',
        );
      }
