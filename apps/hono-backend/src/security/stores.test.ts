...
function createRedisMock() {
  ...
  const mock = {
    ...
    eval: vi.fn(
      async (lua: string, { keys, arguments: args }: { keys: string[]; arguments?: string[] }) => {
        const argv = args ?? [];
        const key = keys[0]!;
        const now = Date.now();
...
describe("rateLimitRedis", () => {
  ...
  it("allows up to capacity requests", async () => {
    for (let i = 0; i < CAPACITY; i++) {
      const result = await rateLimitRedis(redis, KEY, {
        capacity: CAPACITY,
  ...
  it("blocks after capacity is exhausted → HTTP 429", async () => {
    ...
    const blocked = await rateLimitRedis(redis, KEY, {
      capacity: CAPACITY,
      refillPerSecond: REFILL,
    });
